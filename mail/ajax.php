...
if(isset($_POST['ajax'])){
	...
	$inmbueble=htmlspecialchars($_POST['inmbueble']);
	...
	elseif(strlen($apellido)>60){
		$msjStatus = "<script>document.getElementById('apellido-status').innerHTML = 'M&aacute;ximo permitido 60 caracteres';</script>";
	...
	elseif($inmbueble==""){
		$msjStatus = "<script>document.getElementById('r las opciones del select
		elseif(inmbueble-status').innerHTML = 'Debe seleccionar una opci&oacute;n';</script>";
	}
	...
	/*elseif(!preg_match("/^[a-zA-ZáéíóúÁÉÍÓÚ\s]*$/", $mensaje)){
		...
		<head></head>
		<body>
		<h3>'.$nombre.' ha solicitado el 10% en control de plagas Residencial</h3>
		...
		<ul>
			...
			<li><strong>Tipo de Inmueble: </strong> '.$inmueble.'</li>
		...
		if($envio){
			...
			."Su mensaje ha sido recibido satisfactoriamente con los siguientes.\n"
